"""Lightning module for training the DIFUSCO Own model."""
...
from co_datasets.own_dataset import OwnDataset
from utils.diffusion_schedulers import InferenceSchedule
from pl_meta_model import COMetaModel
from utils.own_utils import own_decode_np


class OwnModel(COMetaModel):
  def __init__(self,
               param_args=None):
    super(OwnModel, self).__init__(param_args=param_args, node_feature_only=True)
    ...
    self.validation_dataset = OwnDataset(
  ...
  def test_step(self, batch, batch_idx, draw=False, split='test'):
    ...
    solved_solutions = [own_decode_np(predict_labels, adj_mat) for predict_labels in splitted_predict_labels]
    # solved_costs = [solved_solution.sum()  for solved_solution in solved_solutions]
    solved_costs = [sum(solved_solution == node_labels.cpu().numpy()) for solved_solution in solved_solutions]
    best_solved_cost = np.max(solved_costs)

    gt_cost = len(node_labels.cpu().numpy())  # .sum()
    metrics = {
