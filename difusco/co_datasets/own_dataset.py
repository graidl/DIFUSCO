...
class OwnDataset(torch.utils.data.Dataset):
  ...
  def get_example(self, idx):
    ...
    #   graph = pickle.load(f)  # load the graph idx
    # TODO
    graph = nx.gnm_random_graph(n=100, m=150, seed=1000+idx)
    num_nodes = graph.number_of_nodes()

    # use as node label deg(node)>=3
    degrees = np.array([graph.degree(node) for node in graph.nodes])
    node_labels = (degrees >= 3).astype(np.int64)
